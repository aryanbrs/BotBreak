...
int func4(player &p, int cardSetValue){ //do you have card maximum to current maximum card
    ...
    int HighstCard2 = (HighstCard<100) ? HighstCard%10 : HighstCard%100 ;
    //cout<<current_heighest_card<<' '<<HighstCard<<' '<<HighstCard2<<' '<<Board[5]<<' ';

    if((current_heighest_card <= HighstCard2)  && (Board[Board[5]]<HighstCard)) return func6(p, cardSetValue);
...
int nextPlayer(int wonPlayer){  //return next player
    if(wonPlayer == -1){
        ...
        while(Players->next->value != wonPlayer) Players = Players->next;
        int temp = Players->value;
        Players = Players->next;
        return temp;
    }
...
void sortCards(){  //Used to show cards which are thrown/left and player with their cards
    for(int i=0; i<4; i++){
        for(int j=0; j<4; j++){
...
void control(){
    ...
    while(N){
        ...
        cout<<'P'<<nextPlyr<<'-'<<cardThrown<<"   ";//printing player's played card

        for(int i=0; i<3; i++){
            int j = nextPlayer(-1);
            Board[j]=func1(p[j-1], Board);

            cout<<'P'<<j<<'-'<<Board[j]<<"   ";//printing player's played card
        ...
        cout<<' '<<"Won: "<<'P'<<Board[5];
        cout<<endl<<endl;//Board[0]<<' '<<Board[1]<<' '<<Board[2]<<' '<<Board[3]<<' '<<Board[4]<<' '<<Board[5]<<' '<<endl;
